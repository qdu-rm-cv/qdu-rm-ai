 ...
 public:
  ...
  void extractNumbers(const cv::Mat& src, std::vector<Armor>& armors) {
    ...
    for (auto& armor : armors) {
      ...
      for (int i = 0; i < 4; i++) lights_vertices[i] = image_vertices[i];
  ...
  void classify(std::vector<Armor>& armors, const cv::Mat frame) {
    for (auto& armor : armors) {
      cv::Mat
          image =
              cv::imread(
                  "/home/yang/Desktop/rm_auto_aim/armor_detector/docs/"
                  "num_bin.png");  // armor.number_img_.clone();
    ...
    armors.erase(
        std::remove_if(
            ...
            [this](const Armor& armor) {
              SPDLOG_INFO(
                  "armor.confidence_ :{} armor.number_{} armor.armor_type_:{}",
                  armor.confidence_, armor.number_, armor.armor_type_);
...
class AutoAim : private App {
 ...
 public:
  ...
  void Run() {
    ...
    std::string path1 = kPATH_RUNTIME + "fc.onnx";
    std::string path2 = kPATH_RUNTIME + "label.txt";
    // std::vector<std::string> mask = {"Base", "1", "2",     "3",
    //                                  "4",    "5", "Guard", "Outpost"};
    NumberClassifier num_classfier(path1, path2, 0.3);
    while (1) {
