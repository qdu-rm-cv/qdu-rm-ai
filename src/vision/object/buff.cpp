...
Buff::Buff(const cv::Point2f& center,
           const tbb::concurrent_vector<Armor>& armors, const Armor& target) {
  SetCenter(center);
  SetArmors(armors);
  SetTarget(target);
...
void Buff::SetArmors(const tbb::concurrent_vector<Armor>& armors) {
  armors_ = armors;
}
