...
ArmorDetectorParam<double> ArmorParam::TransformToDouble() {
  ...
  paramd_.contour_area_low_th = parami_.contour_area_low_th / 10000000.;
  paramd_.contour_area_high_th = parami_.contour_area_high_th / 100000.;
  paramd_.bar_area_low_th = parami_.bar_area_low_th / 10000000.;
  ...
  paramd_.center_y_dist = parami_.center_y_dist / 1000.;
...
bool ArmorParam::Read(const std::string &params_path) {
  ...
  if (fs.isOpened()) {
    ...
    parami_.center_y_dist = static_cast<double>(fs["center_y_dist"]) * 1000.;
...
void ArmorParam::Write(const std::string &params_path) {
  ...
  fs << "center_y_dist" << paramd_.center_y_dist;
