...
const Armor& AntiWhippingTop::GetTargetArmor(cv::RotatedRect pre_rect,
  ...
  // TODO(MC) :长宽比还需要数学运算
  float width = (pre_rect.size.width+now_rect.size.width)/1.7;
  float height = (pre_rect.size.height+now_rect.size.height)/1.94;
  float angle = 0;
